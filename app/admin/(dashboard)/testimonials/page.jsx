...
export default function TestimonialsManagementPage() {
  ...
  const columns = [
  {
    ...
    accessor: (row) =>
    <div>
          <p className="font-bold text-on-surface">{row.clientName}</p>
          <p className="text-xs text-on-surface-variant mt-1">{row.clientTitle}{row.company ? ` - ${row.company}` : ""}</p>
        </div>
  ...
  },
  {
    ...
    <div className="flex text-lime">
          {[...Array(5)].map((_, i) =>
      <svg key={i} className={`w-4 h-4 ${i < row.rating ? "text-lime" : "text-outline-variant"}`} fill="currentColor" viewBox="0 0 20 20">
